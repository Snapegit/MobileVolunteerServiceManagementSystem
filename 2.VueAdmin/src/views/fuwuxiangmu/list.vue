	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('fuwuxiangmu','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('fuwuxiangmu','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('fuwuxiangmu','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('fuwuxiangmu','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="封面" width="120" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('fuwuxiangmu','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('fuwuxiangmu','服务预约')" type="success" @click="fuwujiluCrossAddOrUpdateHandler(scope.row,'cross','','zhuangtai','已预约','已预约,未预约'.split(',')[0])">服务预约</el-button>
						<el-button v-if="btnAuth('fuwuxiangmu','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<fuwujiluFormModel ref="fuwujiluFormModelRef" @formModelChange="formModelChange"></fuwujiluFormModel>
	</div>
</template>
<script setup>
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussfuwuxiangmu?refid=' + id)
	}
	import fuwujiluFormModel from '@/views/fuwujilu/formModel'
	const fuwujiluFormModelRef = ref(null)
    const fuwujiluCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			fuwujiluFormModelRef.value.init(row.id,'cross','服务预约',row,'fuwuxiangmu',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
